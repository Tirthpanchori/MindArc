...
import { 
  ArrowLeft, Brain, CheckCircle2, XCircle, Clock, Calendar, Mail, Award, AlertTriangle, Lightbulb, Target, BookOpen, BarChart2, List, Sparkles, RefreshCw, ChevronDown, ChevronUp
} from "lucide-react";
...
function ResultTeacher() {
  ...
  const fetchAiAnalysis = async () => {
    ...
    try {
      ...
      const response = await api.post("ai/analyze-weak-topics/", 
    ...
    } catch (error) { setAnalysisError("Failed to analyze topics."); } 
  ...
  if (!result) return (
    <div className="min-h-screen bg-[#0f1117] flex items-center justify-center font-space-grotesk p-6">
      <div className="text-center">
        ...
        <button onClick={() => navigate(-1)} className="px-5 py-2.5 rounded bg-[#1a1d27] border border-[#374151] text-[#9CA3AF] hover:text-white uppercase tracking-wider text-sm font-bold">Go Back</button>
      </div>
    </div>
  );
  ...
  return (
    <div className="min-h-screen bg-[#0f1117] text-white font-space-grotesk selection:bg-[#F59E0B] selection:text-[#0f1117] p-6 md:p-8 lg:p-12">
      <div className="max-w-5xl mx-auto space-y-8">
        ...
        <button onClick={() => navigate(-1)} className="text-[#9CA3AF] hover:text-white text-sm font-bold flex items-center gap-2 transition-colors uppercase tracking-wider mb-2">
        ...
        <div className="bg-[#1a1d27] border border-[#374151] rounded p-6 md:p-8 flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
          <div>
            ...
            <div className="flex flex-col sm:flex-row sm:items-center gap-4 text-[#9CA3AF] text-xs font-bold uppercase tracking-wider mt-4">
              <span className="flex items-center gap-2">
                <div className="w-6 h-6 rounded bg-[#0f1117] border border-[#374151] flex items-center justify-center text-xs text-white">
                  {student_name?.charAt(0).toUpperCase()}
                </div>
                {student_name}
              </span>
              <span className="hidden sm:inline text-[#374151]">•</span>
              <span className="flex items-center gap-1.5 text-white font-medium lowercase"><Mail size={14} className="text-[#F59E0B]"/> {student_email}</span>
              <span className="hidden sm:inline text-[#374151]">•</span>
              ...
              {results && results.length > 0 ? results.map((q, i) => (
                  <div key={q.question_id ?? i} className="bg-[#1a1d27] border border-[#374151] rounded p-6 hover:border-[#F59E0B] transition-colors">
                    <div className="flex justify-between items-start gap-4 mb-4">
                      ...
                      <span className={`flex-shrink-0 px-2.5 py-1 rounded text-xs font-bold uppercase tracking-wider border flex items-center gap-1.5 ${q.is_correct ? "bg-green-500/5 text-green-500 border-green-500/20" : "bg-red-500/5 text-red-500 border-red-500/20"}`}>
                        {q.is_correct ? "Correct" : "Wrong"}
                      </span>
                    </div>
                    <div className="grid md:grid-cols-2 gap-4 text-sm font-medium">
                      <div className={`p-4 rounded border ${q.is_correct ? "bg-green-500/5 border-green-500/20" : "bg-red-500/5 border-red-500/20"}`}>
                        <p className="text-[#9CA3AF] text-xs font-bold uppercase tracking-widest mb-2">Student's Answer</p>
                        <p className={`${q.is_correct ? "text-green-500" : "text-red-500"}`}>{q.selected_option_text ?? q.selected_option ?? "—"}</p>
                      </div>
                  ...
                  <div className="bg-[#1a1d27] border border-[#374151] rounded p-6 md:p-8">
                    ...
                    {aiAnalysis.total_incorrect === 0 && (
                      <div className="bg-green-500/5 border border-green-500/20 rounded p-4 flex items-center gap-3">
                        ...
                        <div><h4 className="font-bold text-green-500">Perfect Score!</h4><p className="text-sm font-medium text-green-500/80">Great job! Keep challenging the student.</p></div>
                      </div>
                    )}
                  </div>
